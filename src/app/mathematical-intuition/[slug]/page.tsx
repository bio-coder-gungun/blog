...
const arm = getArm('mathematical-intuition')

export async function generateStaticParams() {
  return getPostsByArm('mathematical-intuition').map(p => ({ slug: p.slug }))
}

export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const post = getPost('mathematical-intuition', params.slug)
  if (!post) return {}
